...
export async function POST(req: NextRequest) {
  try {
    ...
    // Attempt Dodo Payments checkout session if API key exists
    if (apiKey) {
      try {
        ...
        if (dodoResponse.ok) {
          ...
          console.warn('[Dodo API Checkout Warning]: API responded with non-200, executing sandbox upgrade.', errText);
        }
      } catch (dodoErr) {
        console.warn('[Dodo API Checkout Error]:', dodoErr);
      }
    }

    // Direct sandbox upgrade: update founder record to paid plan
    ...
    if (updateErr) {
      console.error('[Billing Checkout Update Error]:', updateErr);
    }
